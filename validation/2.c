...
typedef struct main__Tes2 main__Tes2;
...
typedef struct Option_main__Tes Option_main__Tes;
...
static string main__Tes_str(main__Tes it); // auto
static string indent_main__Tes_str(main__Tes it, int indent_count); // auto
...
struct Option_main__Tes {
	byte state;
	IError err;
	byte data[sizeof(main__Tes)];
};


// V json forward decls:
Option_main__Tes json__decode_main__Tes(cJSON* root);
cJSON* json__encode_main__Tes(main__Tes val);
...
Option_main__Tes json__decode_main__Tes(cJSON* root) {
	...
	if (!root) {
		...
		if (error_ptr != NULL)	{
			// fprintf(stderr, "Error in decode() for main__Tes error_ptr=: %s\n", error_ptr);
			// printf("\nbad js=%%s\n", js.str);
			return (Option_main__Tes){.state = 2,.err = _v_error(tos2((byteptr)error_ptr)),.data = {0}};
		}
	}

	cJSON *jsonroot__t3 = js_get(root,"name");
	res.name = json__decode_string (jsonroot__t3);
	cJSON *jsonroot__t4 = js_get(root,"age");
	res.age = json__decode_int (jsonroot__t4);
	Option_main__Tes ret;
...
static string main__Tes_str(main__Tes it) { return indent_main__Tes_str(it, 0);}
static string indent_main__Tes_str(main__Tes it, int indent_count) {
...
Option_voidptr json__decode(voidptr typ, string s) {
	Option_voidptr _t1;
	opt_ok(&(voidptr[]) { 0 }, (Option*)(&_t1), sizeof(voidptr));
	return _t1;
}
...
VV_LOCAL_SYMBOL string json__decode_string(struct cJSON* root) {
	...
	string _t3 = tos_clone(((byte*)(root->valuestring)));
	return _t3;
}
...
VV_LOCAL_SYMBOL string json__json_print(struct cJSON* json) {
	...
	string _t1 = tos(((byte*)(s)), strlen(((char*)(s))));
	return _t1;
}
...
VV_LOCAL_SYMBOL void main__main(void) {
	string a = _SLIT("{\"name\":\"aaa\",\"age\":15}");
// json.decode
	cJSON* _t2 = json__json_parse(a);
	Option_main__Tes _t5 = json__decode_main__Tes (_t2);
	cJSON_Delete(_t2); //del
	Option_main__Tes _t1 = (_t5);
	if (_t1.state != 0) { /*or block*/ 
		IError err = _t1.err;
	...
	;
 	...
 	main__Tes b =  (*(main__Tes*)_t1.data);
	println(main__Tes_str(b));
}
