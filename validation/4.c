...
#if __has_include(<4.h>)
#include <4.h>
#else
#error VERROR_MESSAGE Header file <4.h>, needed for module `main` was not found. Please install the corresponding development headers.
#endif
...
static string Tesffffffffft_str(Tesffffffffft it); // auto
static string indent_Tesffffffffft_str(Tesffffffffft it, int indent_count); // auto
...
static string Tesffffffffft_str(Tesffffffffft it) { return indent_Tesffffffffft_str(it, 0);}
static string indent_Tesffffffffft_str(Tesffffffffft it, int indent_count) {
...
VV_LOCAL_SYMBOL void main__main(void) {
	Tesffffffffft* a = (Tesffffffffft*)memdup(&(Tesffffffffft){.name = (string){.str=(byteptr)"", .is_lit=1},.age = 0,}, sizeof(Tesffffffffft));
	Tesffffffffft b = TTTTtest(a);
	println(str_intp(1, _MOV((StrIntpData[]){{_SLIT("&"), 0xfe10 ,{.d_s=Tesffffffffft_str(*a)}}})));
	println(Tesffffffffft_str(b));
}
